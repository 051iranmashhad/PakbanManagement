...
class PersianDateFormat {
    ...
    fun format(date: PersianDate): String {
        val year2: String = when (("" + date.shYear).length) {
            2 -> {
                "" + date.shYear
            }
    ...
    fun parse(date: String, pattern: String = this.pattern): PersianDate {
        ...
        for (i in keyParse.indices) {
            if (pattern.contains(keyParse[i])) {
                ...
                if (dateReplace.matches(Regex("[-+]?\\d*\\.?\\d+"))) {
    ...
    fun parseGrg(date: String, pattern: String = this.pattern): PersianDate {
        ...
        return PersianDate(dateInGrg?.time ?: 0L)
    }
    ...
    private fun stringUtils(text: String, key: Array<String>, values: Array<String>): String {
        ...
        for (i in key.indices) {
            restext = restext.replace(key[i], values[i])
        }
        return restext
    }
    ...
    companion object {
        ...
        fun format(
            ...
            if (pattern1 == null) pattern1 = "l j F Y H:i:s"
            val key = arrayOf(
                "a",
                "l",
                "j",
