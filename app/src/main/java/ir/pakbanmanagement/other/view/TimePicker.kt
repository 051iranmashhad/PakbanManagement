...
import ir.pakbanmanagement.databinding.LayoutTimePickerBinding
...
class TimePicker @JvmOverloads constructor(
    ...
    private val binding: LayoutTimePickerBinding =
        LayoutTimePickerBinding.inflate(LayoutInflater.from(context), this, true)
    ...
    private fun initConfigurations() {
        context.theme.obtainStyledAttributes(
            attrs, R.styleable.Ticker, defStyleAttr, 0
        ).apply {
            try {
...
//        }
    ...
    private fun initHoursAndMinutesList() {
        ...
        initTickerRecyclerViews(
            binding.rvHours, hoursAdapter, hoursList, true
        )
        initTickerRecyclerViews(
            binding.rvMinutes, minutesAdapter, minutesList, false
        )
            ...
            override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                    updateTextColorForSelectedTimeText(
                        recyclerView, ir.pakbanmanagement.R.color.color_black, isHours
                    )
                } else {
                    updateTextColorForSelectedTimeText(
                        recyclerView, ir.pakbanmanagement.R.color.color_gray_dark, isHours
                    )
                }
            }

            private fun updateTextColorForSelectedTimeText(
                recyclerView: RecyclerView, @ColorRes textColorRes: Int, isHours: Boolean,
            ) {
    ...
    private fun updateCurrentlySelectedValues(
        isHours: Boolean, timeString: String, @ColorRes appliedColorRes: Int,
    ) {
    ...
    fun getCurrentlySelectedTime(format: String = "HH:MM FORMAT"): String {
        ...
        return format.replace("HH", currentlySelectedHour).replace("MM", currentlySelectedMinute)
